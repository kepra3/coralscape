...
Overhang - presence and absense at the moment but will be % cover.
...
Theta - angle of attachment. Might have interaction with overhang and colony rugosity.

```{r angle of attachment}
...
Outcrop position - proportion of height the colony sits at in relation to it's local environment.
...
Raw depth, local depth measures within plots. Comparisons between plots don't make sense but comparisons with are real depth differences.

```{r local z}
k_boxplot(metadata, metadata$Species, metadata$z) + ggtitle('Relative depth')
...
## Overhang
Presence and absence thus a binomial family.
```{r}
...
Linear model 1 - with mixed effects.
```{r}
# mixed model
...
Linear model 2 - with mixed effects.
```{r}
# mixed model 2
