...
class Viscore_metadata(object):
    ...
    def __init__(self, subsets_filename, short_name):
        subsets = json.load(open('../subsets/{}'.format(subsets_filename)))
...
def main(ply_filename, annotations_filename, subsets_filename, path):
    ...
    pcd = o3d.io.read_point_cloud('{}/{}_dec50M.ply'.format(path, ply_filename))
    ...
    annotations = get_annotations('../raw_annotations/{}'.format(annotations_filename))
    ...
    ranges = get_ranges('../raw_annotations/{}'.format(annotations_filename), annotations, viscore_md.scale_factor)
...
if __name__ == '__main__':
    ...
    # break up ply_filename
    break_name = ply_filename.split('_')[0:4]
    dir1 = "_".join(break_name[0:2])
    dir2 = "_".join(break_name[0:3])

    subsets_filename = "{}_subsets.json".format(ply_filename)
